...
fdr_output.ref <- file.path(
  "data", "2_INTERIM", "OUTPUT-REFERENCE_PARQUET"
)
...
tar_option_set(
  ...
  controller = crew_controller_local(
    ...
    options_metrics = crew_options_metrics(
      path = "logs/metrics_main/",
      seconds_interval = 1
    ),
    options_local = crew_options_local(
      log_directory = "logs/workers_main/"
    )
  )
...
# we can refer to "summary_pipeline_main.html" generated by the pipeline.

# If for some reason the pipeline stops completely and "summary_pipeline_main.html"
...
if (tar_active()) {
  log_start(
    path = file.path("logs", "pipeline_main.log"), # Statistics on the main process go here.
    seconds = 1
  )
}
