...
3.  Install compilation tools (platform-specific)
    ...
    **macOS:**
    ...
    Several R packages need to be compiled from source. Install the following dependencies via [Homebrew](https://brew.sh/):

    ```
    brew install cmake gcc gettext
    ```

    Then create `~/.R/Makevars` so R can find the installed libraries:
    ...
    Additionally, the `arrow` R package requires setting an environment variable before running `renv::restore()`:
...
-   **Resetting conda environments:** If the config pipeline errors at `lst_miniconda` or the `summary_miniconda.html` report shows environments highlighted red, removing the problematic environment and re-running the config pipeline will recreate it from scratch. Run the following in your system terminal (not the R console), replacing the environment name as needed:

    ```
    conda env remove -n WHO_WAVES_oak_pre -y
    ```

    Available environment names: `WHO_WAVES_stepcount`, `WHO_WAVES_accelerometer`, `WHO_WAVES_actinet`, `WHO_WAVES_oak_pre`, `WHO_WAVES_oak_1.0`

    To remove all WAVES environments at once:

    ```
    conda env list | grep WHO_WAVES | awk '{print $1}' | xargs -I {} conda env remove -n {} -y
    ```
