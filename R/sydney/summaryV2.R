...
summary<-function(ag,win=15){
  ...
  for(i in 1:length(a)){
    ...
    p = which(final$class==3|final$class==4|final$class==4); rr1[p] = 1
  ...
  for(i in 1:length(a)){
    ...
    mat[z,17]<-aa$mvpa;mat[z,18]<-aa$mvpabouts;mat[z,19]<-aa$mvpaboutsavg;mat[z,20]<-aa$mvpamaxdura;mat[z,21]<-aa$mvpamaxdurast[1];mat[z,22]<-aa$mvpamaxduraend[1]
    mat[z,23]<-aa$mvpamindura;mat[z,24]<-aa$mvpamindurast[1];mat[z,25]<-aa$mvpaminduraend[1]
    mat[z,26]<-aa$sedb;mat[z,27]<-aa$sedbouts;mat[z,28]<-aa$sedboutsavg;mat[z,29]<-aa$sedmaxdura;mat[z,30]<-aa$sedmaxdurast[1];mat[z,31]<-aa$sedmaxduraend[1]
    mat[z,32]<-aa$sedmindura;mat[z,33]<-aa$sedmindurast[1];mat[z,34]<-aa$sedminduraend[1]
    ...
    mat[z,42]<-aa$nw.mg;mat[z,43]<-aa$nw.mgvm
