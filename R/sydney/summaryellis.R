...
summary<-function(ag,win=15){
  ...
  for(i in 1:length(a)){
    ...
    class.biking<-round(as.numeric(length(which(final$class%in%"Biking")))*win/60,2)
    class.sed<-round(as.numeric(length(which(final$class%in%"Sedentary")))*win/60,2)
    class.standmov<-round(as.numeric(length(which(final$class%in%"StandingMoving")))*win/60,2)
    class.standstill<-round(as.numeric(length(which(final$class%in%"StandingStill")))*win/60,2)
    class.walk<-round(as.numeric(length(which(final$class%in%"Walking")))*win/60,2)
    class.vehicle<-round(as.numeric(length(which(final$class%in%"Vehicle")))*win/60,2)
    ...
    nw.mgvm<-round(mean(final$mean[which(final$class==7)]),5)
    
    boutduration = 10 * (60/win) #per 10 minutes
    #boutduration = mvpadur * (60/win)##################added on 07/09/2019
    rr1 = matrix(0,length(final$intensity),1)
    
    p = which(final$class%in%"Walking"|final$class%in%"Biking"); rr1[p] = 1
    ...
    p = which(final$class%in%"Sedentary"|final$class%in%"Vehicle"); rr1[p] = 1
    
    #boutduration = seddur * (60/win) #################added on 09/07/2019
    ...
    see[[i]]<-list(int.sed = int.sed,int.light = int.light,int.moderate = int.moderate,int.vigorous = int.vigorous,
                   class.biking = class.biking,class.sed = class.sed,class.standmov = class.standmov,class.standstill = class.standstill, class.walk = class.walk,class.vehcile=class.vehicle,
  ...
  for(i in 1:length(a)){
    ...
    mat[z,9]<-aa$class.biking;mat[z,10]<-aa$class.sed;mat[z,11]<-aa$class.standmov;mat[z,12]<-aa$class.standstill;mat[z,13]<-aa$class.walk
    mat[z,14]<-aa$sleep;mat[z,15]<-aa$sleepperiod;mat[z,16]<-aa$nw
    mat[z,4]<-ifelse(mat[z,4]%in%c("Saturday","Sunday"),0,1)
    mat[z,17]<-aa$mvpa;mat[z,18]<-aa$mvpabouts;mat[z,19]<-aa$mvpaboutsavg;mat[z,20]<-aa$mvpamaxdura;mat[z,21]<-aa$mvpamaxdurast[1]
    mat[z,22]<-aa$mvpamaxduraend[1];mat[z,23]<-aa$mvpamindura;mat[z,24]<-aa$mvpamindurast[1];mat[z,25]<-aa$mvpaminduraend[1]
    mat[z,26]<-aa$sedb;mat[z,27]<-aa$sedbouts;mat[z,28]<-aa$sedboutsavg;mat[z,29]<-aa$sedmaxdura;mat[z,30]<-aa$sedmaxdurast[1]
    mat[z,31]<-aa$sedmaxduraend[1];mat[z,32]<-aa$sedmindura;mat[z,33]<-aa$sedmindurast[1];mat[z,34]<-aa$sedminduraend[1]
    mat[z,35]<-as.character(aa$start);mat[z,36]<-as.character(aa$start.day)
    if(length(aa$end)==0){aa$end<-NA}
    mat[z,37]<-as.character(aa$end)
    if(length(aa$end.day)==0){aa$end.day<-NA}
    mat[z,38]<-as.character(aa$end.day)
    mat[z,39]<-aa$motionless;mat[z,40]<-aa$light.s;mat[z,41]<-aa$restless
    mat[z,42]<-aa$nw.mg;mat[z,43]<-aa$nw.mgvm;mat[z,44]<-aa$class.vehcile
