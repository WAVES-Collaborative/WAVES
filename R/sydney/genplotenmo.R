...
GenPlot<-function(final,window=win,output,folder_name=folder_name,file_name=file_name)
{
  ...
  for(i in 1:length(a)){
    ...
    try(for(zzz in 1:2){
      ...
      e<-which(see$intensity_comb%in%"1") #this extracts only indices that are activity=1
      ...
      e<-which(see$intensity_comb%in%"2")
      d[e]<-"blue"
      ...
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$intensity_comb%in%"3")
      d[e]<-"orange"
      ...
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$intensity_comb%in%'4')
      d[e]<-"purple"
      ...
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$intensity_comb==6)
      d[e]<-"skyblue"
      
      for (iii in 1:dim(see)[1])
      {
        
        rug(iii,side=1,col=d[iii],line=-3.9,lwd=.001)
        
      }
      ##############
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$intensity_comb==8)
      d[e]<-"red"
      ...
      text(y = labels,x=1, par("usr")[1]-.07, labels = c("Sed","Light","Mod","Vig","Sleep","Nap","NW"), srt = 0, pos = 2, xpd = TRUE,cex=.60) #puts class labels next to rug plot
      ##############
      ###############PLOT MVPA BOUTS
      ...
      p = which(see$intensity_comb==3|see$intensity_comb==4); rr1[p] = 1
      ...
      p = which(see$intensity_comb==1); rr1[p] = 1
    ...
    if(i==length(a)){
      for(ss in 1:length(a)){
        ...
        e<-as.numeric(length(which(final3$intensity_comb==1)))
        e<-round(e*window/60,2)
        rp[4]<-paste("Time spent in SED activities:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$intensity_comb==2)))
        e<-round(e*window/60,2)
        rp[3]<-paste("Time spent in light activities:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$intensity_comb==3)))
        e<-round(e*window/60,2)
        rp[2]<-paste("Time spent in mod activities:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$intensity_comb==4)))
        e<-round(e*window/60,2)
        ...
        if(sss<=length(a)){
          ...
          e<-round(e*window/60,2)
          rp[5]<-paste("Time spent Sleeping:",e, "mins" ,sep = " ")
        }
        if(sss>length(a)){rp[4]<-"No Sleep On Last Day"}
        e<-as.numeric(length(which(final3$intensity_comb==8)))
        e<-round(e*window/60,2)
        rp[4]<-paste("Nap:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$intensity_comb==7)))
        e<-round(e*window/60,2)
        ...
        p = which(final3$intensity_comb==3|final3$intensity_comb==4); rr1[p] = 1
        gbo<-getbout(rr1,boutduration = boutduration)
        ...
        p = which(final3$intensity_comb==1); rr1[p] = 1
        gbo<-getbout(rr1,boutduration = boutduration,boutcriter = .9)
        e<-as.numeric(length(which(gbo$x == 1)))
        e<-round(e*window/60,2)
        rp[1]<-paste("SED bout:",e, "mins" ,sep = " ")
        legend(1,13, bty="n", legend=rp,cex=2) 
      }}
  }
