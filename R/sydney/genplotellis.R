...
GenPlot<-function(final,window=win,output,folder_name=folder_name,file_name=file_name)
{
  ...
  for(i in 1:length(a)){
    ...
    try(for(zzz in 1:2){
      ...
      e<-which(see$class%in%"Biking") #this extracts only indices that are activity=1
      ...
      e<-which(see$class%in%"Sedentary")
      d[e]<-"blue"
      ...
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$class%in%"StandingMoving")
      d[e]<-"orange"
      ...
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$class%in%"StandingStill")
      d[e]<-"purple"
      ...
      d<-rep(NA,dim(see)[1])
      e<-which(see$class%in%"Vehicle")
      d[e]<-"blue"
      
      for (iii in 1:dim(see)[1])
      {
        
        rug(iii,side=1,col=d[iii],line=-.85,lwd=.001)
      ...
      text(y = labels,x=1, par("usr")[1]-.07, labels = c("BIKE","SED","S+","S","WALK","Sleep","NW"), srt = 0, pos = 2, xpd = TRUE,cex=.60) #puts class labels next to rug plot
      ##############
      ###############PLOT MVPA BOUTS
      boutduration = 10 * (60/window) #per 10 minutes
      #boutduration = mvpadur * (60/window)##################added on 07/09/2019
      rr1 = matrix(0,length(see$enmo),1)
      
      p = which(see$class%in%"Walking"|see$class%in%"Biking"); rr1[p] = 1
      ...
      #boutduration = seddur * (60/window) #################added on 09/07/2019
      rr1 = matrix(0,length(see$enmo),1)
      
      p = which(see$class%in%"Sedentary"|see$class%in%"Vehicle"); rr1[p] = 1
    ...
    if(i==length(a)){
      for(ss in 1:length(a)){
        ...
        e<-as.numeric(length(which(final3$class%in%"Biking")))
        e<-round(e*window/60,2)
        rp[5]<-paste("Time spent cycling:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$class%in%"Sedentary")))
        e<-round(e*window/60,2)
        rp[4]<-paste("Time spent in SED activities:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$class%in%"StandingStill")))
        e<-round(e*window/60,2)
        rp[3]<-paste("Time spent standing still:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$class%in%"StandingMoving")))
        e<-round(e*window/60,2)
        rp[2]<-paste("Time spent standing +:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$class%in%"Walking")))
        e<-round(e*window/60,2)
        rp[1]<-paste("Time spent walking:",e, "mins" ,sep = " ")
        ...
        p = which(final3$class%in%"Walking"|final3$class%in%"Biking"); rr1[p] = 1
        gbo<-getbout(rr1,boutduration = boutduration, ws3 = window)
        ...
        p = which(final3%in%"Sedentary"|final3%in%"Vehicle"); rr1[p] = 1
        gbo<-getbout(rr1,boutduration = boutduration,boutcriter = .9, ws3 = window)
        e<-as.numeric(length(which(gbo$x == 1)))
        e<-round(e*window/60,2)
