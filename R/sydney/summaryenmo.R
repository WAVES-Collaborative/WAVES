...
summary<-function(ag,win=10){
  day<-weekdays(as.Date(ag$date)) #this will be used when wanting to summarise data by day of week
  ag<-cbind(ag,day)
  allday<-ag[,c(1:3,52,53,55:59)]
  ag<-ag[,c(1:3,52,53,55:59)]
  
  
  a<-unique(ag$date)
  mat<-matrix("",length(a),38)
  see<-list()
 
  for(i in 1:length(a)){
    final<-ag[ag$date%in%a[i],]
    int.sed<-round(as.numeric(length(which(final$intensity_comb==1)))*win/60,2)
    int.light<-round(as.numeric(length(which(final$intensity_comb==2)))*win/60,2)
    int.moderate<-round(as.numeric(length(which(final$intensity_comb==3)))*win/60,2)
    int.vigorous<-round(as.numeric(length(which(final$intensity_comb==4)))*win/60,2)
    ...
    if(ii<=length(a)){
      ...
      x<-x-1
      final4<-final4[z:x,]
      e<-as.numeric(length(which(final4$intensity_comb==6)))
      sleep<-round(e*win/60,2)
      e<-which(final4$intensity_comb==6)
    ...
    nw<-round(as.numeric(length(which(final$intensity_comb==7)))*win/60,2)
    nw.mg<-round(mean(final$enmo[which(final$intensity_comb==7)]*1000),5)
    nw.mgvm<-round(mean(final$Mean.Y[which(final$intensity_comb==7)]),5)
    sleep.nap<-round(as.numeric(length(which(final$intensity_comb==8)))*win/60,2)
    
    boutduration = 10 * (60/win) #per 10 minutes
    
    rr1 = matrix(0,length(final$intensity_comb),1)
    
    p = which(final$intensity_comb==3|final$intensity_comb==4); rr1[p] = 1
    ...
    p = which(final$intensity_comb==1); rr1[p] = 1
    ...
    see[[i]]<-list(int.sed = int.sed,int.light = int.light,int.moderate = int.moderate,int.vigorous = int.vigorous,
                   sleep = sleep,sleep.nap = sleep.nap, nw = nw, nw.mg = nw.mg, nw.mgvm = nw.mgvm, date = final$date[1], day = final$day[1],ID = final$subject[1],mvpa = mvpa, sedb = sedb,
  ...
  for(i in 1:length(a)){
    z<-z+1
    aa<-see[[i]]
    mat[z,1]<-as.character(aa$ID);mat[z,2]<-as.character(aa$date);mat[z,3]<-as.character(aa$day);mat[z,4]<-as.character(aa$day);
    mat[z,5]<-aa$int.sed;mat[z,6]<-aa$int.light;mat[z,7]<-aa$int.moderate;mat[z,8]<-aa$int.vigorous
    mat[z,9]<-aa$sleep;mat[z,10]<-aa$sleep.nap;mat[z,11]<-aa$nw
    mat[z,4]<-ifelse(mat[z,4]%in%c("Saturday","Sunday"),0,1)
    mat[z,12]<-aa$mvpa;mat[z,13]<-aa$mvpabouts;mat[z,14]<-aa$mvpaboutsavg;mat[z,15]<-aa$mvpamaxdura;mat[z,16]<-aa$mvpamaxdurast[1];mat[z,17]<-aa$mvpamaxduraend[1];
    mat[z,18]<-aa$mvpamindura;mat[z,19]<-aa$mvpamindurast[1];mat[z,20]<-aa$mvpaminduraend[1]
    mat[z,21]<-aa$sedb;mat[z,22]<-aa$sedbouts;mat[z,23]<-aa$sedboutsavg;mat[z,24]<-aa$sedmaxdura;mat[z,25]<-aa$sedmaxdurast[1];mat[z,26]<-aa$sedmaxduraend[1];
    mat[z,27]<-aa$sedmindura;mat[z,28]<-aa$sedmindurast[1];mat[z,29]<-aa$sedminduraend[1]
    mat[z,30]<-as.character(aa$start);mat[z,31]<-as.character(aa$start.day)
    if(length(aa$end)==0){aa$end<-NA}
    mat[z,32]<-as.character(aa$end)
    if(length(aa$end.day)==0){aa$end.day<-NA}
    mat[z,33]<-as.character(aa$end.day)
    mat[z,34]<-aa$motionless;mat[z,35]<-aa$light.s;mat[z,36]<-aa$restless
    mat[z,37]<-aa$nw.mg;mat[z,38]<-aa$nw.mgvm
