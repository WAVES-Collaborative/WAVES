...
GenPlot<-function(final,window=win,output,folder_name=folder_name,file_name=file_name)
{
  ...
  for(i in 1:length(a)){
    ...
    try(for(zzz in 1:2){
      ...
      e<-which(see$class%in%"1") #this extracts only indices that are activity=1
      ...
      text(y = labels,x=1, par("usr")[1]-.07, labels = c("Sed","SPlus","Walk","Run","","Sleep","NW"), srt = 0, pos = 2, xpd = TRUE,cex=.60) #puts class labels next to rug plot
      ##############
      ###############PLOT MVPA BOUTS
      ...
      p = which(see$class%in%"3"|see$class%in%"4"); rr1[p] = 1
      ...
      p = which(see$class%in%"1"); rr1[p] = 1
    ...
    if(i==length(a)){
      for(ss in 1:length(a)){
        ...
        e<-as.numeric(length(which(final3$class%in%"1")))
        e<-round(e*window/60,2)
        rp[4]<-paste("Time spent in SED activities:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$class%in%"2")))
        e<-round(e*window/60,2)
        rp[3]<-paste("Time spent SUM:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$class%in%"3")))
        e<-round(e*window/60,2)
        rp[2]<-paste("Time spent walking:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$class%in%"4")))
        e<-round(e*window/60,2)
        ...
        p = which(final3$class%in%"3"|final3$class%in%"4"); rr1[p] = 1
        gbo<-getbout(rr1,boutduration = boutduration, ws3 = window)
        ...
        p = which(final3$class%in%"1"); rr1[p] = 1
        gbo<-getbout(rr1,boutduration = boutduration,boutcriter = .9, ws3 = window)
        e<-as.numeric(length(which(gbo$x == 1)))
        e<-round(e*window/60,2)
