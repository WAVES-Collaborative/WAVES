...
partialdaysummaryellis<-function(ag,win=15){
  ...
  mat[1,9]<-"class.biking";mat[1,10]<-"class.sed";mat[1,11]<-"class.standmov";mat[1,12]<-"class.standstill";mat[1,13]<-"class.walk"
  ...
  mat[1,41]<-"nw.mg";mat[1,42]<-"nw.mgvm";mat[1,43]<-"class.vehicle"
  z<-0
  ...
  for(i in 1:length(a)){
    ...
    mat[z,16]<-aa$mvpa;mat[z,17]<-aa$mvpabouts;mat[z,18]<-aa$mvpaboutsavg;mat[z,19]<-aa$mvpamaxdura;mat[z,20]<-aa$mvpamaxdurast[1];mat[z,21]<-aa$mvpamaxduraend[1];mat[z,22]<-aa$mvpamindura;mat[z,23]<-aa$mvpamindurast[1];mat[z,24]<-aa$mvpaminduraend[1]
    mat[z,25]<-aa$sedb;mat[z,26]<-aa$sedbouts;mat[z,27]<-aa$sedboutsavg;mat[z,28]<-aa$sedmaxdura;mat[z,29]<-aa$sedmaxdurast[1];mat[z,30]<-aa$sedmaxduraend[1];mat[z,31]<-aa$sedmindura;mat[z,32]<-aa$sedmindurast[1];mat[z,33]<-aa$sedminduraend[1]
    ...
    mat[z,41]<-aa$nw.mg;mat[z,42]<-aa$nw.mgvm;mat[z,43]<-aa$class.vehcile
