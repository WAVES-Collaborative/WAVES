features<-function(output,win=15,axis="yaxis",Sed=200,Mod=420,Vig=842,Calibrate=F,visual=T,Guide=T,sleep=T,
               ...
               
{
 cat("new\r")
  ...
  #list.of.packages <- c("plyr", "dplyr","e1071","zoo","data.table","randomForest","readxl","Rcpp","HMM","rattle","GENEAread","signal","GGIR","TLBC","DescTools","tools")
...
#load(paste(mypath,"/func.RData",sep=""))
  ...
  for (i in 1:length(part$ID))
  {
   ...
   cat(paste("Finished Reading In .GT3X Accelerometer Data","\n",sep=" "))
    ...
    if(bindata%in%"bin"){ #2020/19/11 NEED TO UPDATE TO MATCH TIMESTAMP SETTING WITH OTHER MONITORS
      hip.data<-list()
      hip.data$header<-data.frame(1,1)
      zz<-GENEAread::header.info(AG)
      sf<-as.numeric(unlist(zz$Value[2]))
      hip.data$header$Sample_Rate<-sf
      hip.data$data<-readGA(AG)
      
    }
    if(bindata%in%"cwa"){
      hip.data<-readax3(AG) #2020/19/11 AX3 has temp, battery, and light data. These have been removed
                            #to save memory space and speed up reading in data. Can be inlcluded in the future if needed
      hip.data$header<-data.frame(1,1)
      hip.data$header$Sample_Rate<-hip.data$Fs              
    }
##############################
...
###### 2020/19/11 commented out. May eventually delete altogether
    # if(Calibrate==T & CalibrateMethod%in%"VH"){
    #   source(paste(mypath,"/mattcalibrate_VH.R",sep = ""))
    #   ag<-calibrate(hip.data$data,temp=F,sphere = 0.3,Fs=hip.data$header$Sample_Rate,chunksize=1,mintime=84)
    #   if(ag$vm.error.end<ag$vm.error.st & ag$vm.error.end<50 & ag$tel==3)
    #   {
    #     start = increment = constant = (hip.data$header$Sample_Rate*60*60*24) #variables used to read data in 24 hr increment
    #     LD<-0
    #     LD2<-0
    ...
    #         LD2<-1
    #       }
    #       cat(paste("\rCalibrating values for hours",round((1+(start *(chunk -1)))/hip.data$header$Sample_Rate/3600,2),"to",round(increment/hip.data$header$Sample_Rate/3600,2),
    #                 "out of",round(chunk2/hip.data$header$Sample_Rate/3600,2),"\r", sep = " "))
    #      
    #       hip.data$data[(1+(start *(chunk -1))):(increment) ,2:4] = 
    #         scale(hip.data$data[(1+(start *(chunk -1))):(increment) ,2:4], center = -ag$offset, 
    #               scale = 1/ag$scale)
    #      
    #       count = count + (increment)
    #       increment = increment+constant
    ...
    #   }
    #   if(ag$vm.error.end>=ag$vm.error.st | !ag$vm.error.end<50){
    #     cat("\nCalibration Results Not Implemented Because Values Do Not Decrease Error\n")
    #   }
    #   if(!ag$tel==3){
    #     cat("\nNot Enough Points Around Sphere To Reliably Perform Calibration\n")
    #   }
    # }
    if(Calibrate==F){
    ...
    cat("\n")
################################# Preschool RF and LVAY wrist RF
    ...
    classifier<-ActivityClassifier(raw=hip.data$data,Fs=hip.data$header$Sample_Rate,ID=ID,
...
################################# Trost Adult RF Wrist 19/06/2019
    ...
    ################################# Thigh Decision Tree 09/10/2020
...
#########################    
    ...
    if(Classifier%in%"Only do enmo"){
      ...
      comb<-summary(comb,win=win)
      colnames(comb$mat)<-c("File","Date","Day","Weekday","int.Sed","int.Light","int.Mod","int.Vig",
                            "Sleep","Sleep.Period","Nap","Non-wear","mvpa.b",'mvpa.bouts','mvpa.b.avg','mvpa.b.max',
