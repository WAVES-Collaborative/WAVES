# https://github.com/wadpac/GGIR/blob/main/user-scripts/verisense_count_steps.R
wrist_steps <- function(input_data,fs,win=10,
  ...
  if (sd(acc) < 0.025) {
    ...
    if ( length(peak_info)==0 || length(peak_info) == sum(is.na(peak_info)) || no_steps == TRUE) {
      ...
      if (length(peak_info)==0) {
        ...
        start_idx_vec <- seq(from=1,to=length(acc),by=fs*win)
