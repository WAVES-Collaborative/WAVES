...
GenPlot<-function(final,window=win,output,folder_name=folder_name,file_name=file_name)
{
  ...
  for(i in 1:length(a)){
    ...
    try(for(zzz in 1:2){
      ...
      if(dim(see)[1]>0)
      {
        plot(see$enmo*1000, main=paste(dat,a[i]," ",see$time[1],"-",see$time[dim(see)[1]],sep = " "),ylim=range(-2500,1800),axes=F,type="l",xlab= "time",ylab = "ENMO",cex.lab=1.5,cex.axis=1.5) #"ylim" should be made to the min and max values of the feature being plotted
      ...
      e<-which(see$class%in%"sit" & see$nonwear==0) #this extracts only indices that are activity=1
      ...
      e<-which(see$nonwear==6 & see$sleep%in%'s')
      d[e]<-"blue"
      ...
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$class%in%"stand" & see$nonwear==0)
      d[e]<-"orange"
      ...
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$class%in%"move" & see$nonwear==0)
      d[e]<-"purple"
      ...
      d<-rep(NA,dim(see)[1])
      e<-which(see$class%in%"walk" & see$nonwear==0)
      d[e]<-"red"
      ...
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$class%in%"run" & see$nonwear==0)
      d[e]<-"skyblue"
      ...
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$class%in%"cycle" & see$nonwear==0)
      d[e]<-"green"
      
      for (iii in 1:dim(see)[1])
      {
        
        rug(iii,side=1,col=d[iii],line=-5.9,lwd=.001)
        
      }
      ##############
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$class%in%"stairs" & see$nonwear==0)
      d[e]<-"blue"
      
      for (iii in 1:dim(see)[1])
      {
        
        rug(iii,side=1,col=d[iii],line=-7,lwd=.001)
        
      }
      ##############
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$nonwear==1)
      d[e]<-"black"
      
      for (iii in 1:dim(see)[1])
      {
        
        rug(iii,side=1,col=d[iii],line=-7.9,lwd=.001)
        
      }
      ##############
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$sleep%in%"sleep")
      d[e]<-"red"
      
      for (iii in 1:dim(see)[1])
      {
        
        rug(iii,side=1,col=d[iii],line=-8.4,lwd=.001)
        
      }

      labels <- c(-2580,-2350,-2050,-1750,-1430,-1230,-950,-650,-400,-100) #this indicates where on the Y axis to put class labels, and will need to be adjusted depending on how many classes there are in a model
      
      text(y = labels,x=1, par("usr")[1]-.07, labels = c("Sit","Lay","Stand","Move","Walk",
                                                         "Run","Cycle","Stairs","NW","Sleep"), srt = 0, pos = 2, xpd = TRUE,cex=.60) #puts class labels next to rug plot
      ##############
      ###############PLOT MVPA BOUTS
      ...
      p = which(see$intensity==3|see$intensity==4); rr1[p] = 1
      ...
      rr1 = matrix(0,length(see$enmo),1)
    table(see$sleep)
      p = which(see$class%in%"sit" & !see$sleep%in%'sleep' & !see$nonwear==1); rr1[p] = 1
      gbo<-getbout(rr1,boutduration = boutduration,boutcriter = .9)
      d<-rep(NA,dim(see)[1])
      e<-which(gbo$x==1)
      d[e]<-"darkgreen"
      ...
      labels <- c(1800,1100) #this indicates where on the Y axis to put class labels, and will need to be adjusted depending on how many classes there are in a model
    ...
    if(i==length(a)){
      for(ss in 1:length(a)){
        ...
        box()
        rp = vector('expression',4) #These next few lines will sum up time spent in min for each class and put on plot
        e<-as.numeric(length(which(final3$intensity==1)))
        e<-round(e*window/60,2)
        rp[4]<-paste("Time spent in SED activities:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$intensity==2)))
        e<-round(e*window/60,2)
        rp[3]<-paste("Time spent in light activities:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$intensity==3)))
        e<-round(e*window/60,2)
        rp[2]<-paste("Time spent in mod activities:",e, "mins" ,sep = " ")
        e<-as.numeric(length(which(final3$intensity==4)))
        e<-round(e*window/60,2)
        rp[1]<-paste("Time spent in vig activities:",e, "mins" ,sep = " ")
        legend("topleft", bty="n", legend=rp,cex=2)
        ...
        e<-as.numeric(length(which(final3$nonwear==1)))
        e<-round(e*window/60,2)
        ...
        p = which(final3$intensity==3|final3$intensity==4); rr1[p] = 1
        gbo<-getbout(rr1,boutduration = boutduration)
        ...
        p = which(final3$intensity==1); rr1[p] = 1
        gbo<-getbout(rr1,boutduration = boutduration,boutcriter = .9)
        e<-as.numeric(length(which(gbo$x == 1)))
        e<-round(e*window/60,2)
        rp[1]<-paste("SED bout:",e, "mins" ,sep = " ")
        legend("topright", bty="n", legend=rp,cex=2) 
      }}
  }
