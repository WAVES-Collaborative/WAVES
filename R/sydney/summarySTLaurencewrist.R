...
summary<-function(ag,win=10){
  ...
  for(i in 1:length(a)){
    ...
    class.standmov<-round(as.numeric(length(which(final$class%in%"Standmov")))*win/60,2)
    class.activegame<-round(as.numeric(length(which(final$class%in%"ActiveGame")))*win/60,2)
    class.walk<-round(as.numeric(length(which(final$class%in%"Walking")))*win/60,2)
    class.run<-round(as.numeric(length(which(final$class%in%"Running")))*win/60,2)
    ...
    see[[i]]<-list(int.sed = int.sed,int.light = int.light,int.moderate = int.moderate,int.vigorous = int.vigorous,
                   class.sed = class.sed,class.standmov = class.standmov,class.activegame = class.activegame,class.walk = class.walk, class.run = class.run,
  ...
  mat[1,9]<-"class.Sed";mat[1,10]<-"class.standmov";mat[1,11]<-"class.activegame";mat[1,12]<-"class.walk";mat[1,13]<-"class.run"
  ...
  for(i in 1:length(a)){
    ...
    mat[z,9]<-aa$class.sed;mat[z,10]<-aa$class.standmov;mat[z,11]<-aa$class.activegame;mat[z,12]<-aa$class.walk;mat[z,13]<-aa$class.run
