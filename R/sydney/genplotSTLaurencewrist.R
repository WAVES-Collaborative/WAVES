...
GenPlot<-function(final,window=win,output,folder_name=folder_name,file_name=file_name)
{
  ...
  for(i in 1:length(a)){
    ...
    try(for(zzz in 1:2){
      ...
      e<-which(see$class%in%"Sedentary") #this extracts only indices that are activity=1
      ...
      ###############
      d<-rep(NA,dim(see)[1])
      e<-which(see$class%in%"ActiveGame")
      d[e]<-"orange"
      ...
      text(y = labels,x=1, par("usr")[1]-.07, labels = c("SED","SPlus","ActiveGame","Walk","Run","Sleep","NW"), srt = 0, pos = 2, xpd = TRUE,cex=.60) #puts class labels next to rug plot
      ##############
      ###############PLOT MVPA BOUTS
      ...
      p = which(see$intensity==1); rr1[p] = 1
